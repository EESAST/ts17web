...
inline double PLDistance(Position &point, Position &line_point_1, Position &line_point_2) {
...
inline double ValueFunc(const Object *obj, const PlayerObject *player, int dist) {
	double decrease = ValueDecreaseRate[dist];
	if (obj->type == ENERGY) {
		return double(kFoodHealth) * decrease;
	} else if (obj->type == ADVANCED_ENERGY) {
		return 1000. * decrease;
	} else if (obj->type == DEVOUR) {
		return -player->max_health * kDevourDamage * 1.5 * decrease;
...
void MonteCarloMove(const Map *map, const PlayerObject *player_now) {
	...
	double max_value = 0;
	Speed best_speed = { rand() % 1000 - 500, rand() % 1000 - 500, rand() % 1000 - 500 };
	...
	for (int i = 0; i != try_times; ++i) {
		...
		if (!CAN_REACH(des, player->radius)) continue;
...
void TryUpgradeSkill(const PlayerObject *player) {
	...
	if (CanUpgrade(player, DASH)) {
		upgrade.value = 10000;
		upgrade.skill = DASH;
		actions.push_back(upgrade);
	} else if (CanUpgrade(player, HEALTH_UP) && player->health < 5000) {
		upgrade.value = 10000;
		upgrade.skill = HEALTH_UP;
		actions.push_back(upgrade);
	} else if (CanUpgrade(player, SHORT_ATTACK)) {
		upgrade.value = 10000;
		upgrade.skill = SHORT_ATTACK;
		actions.push_back(upgrade);
	} else if (CanUpgrade(player, SHIELD) && player->skill_level[DASH] == kMaxSkillLevel) {
		upgrade.value = 10000;
...
bool NoOtherPlayerInSight(const Map *map) {
	for (int i = 0; i != map->objects_number; ++i) {
		auto obj = &map->objects[i];
		if (obj->type == PLAYER && obj->team_id != MyTeam) {
			return false;
		}
	}
	return true;
...
void TryUseSkill(const Map *map, const PlayerObject *player) {
	...
	double nearest_enemy_dis = 100000000;
	...
	for (int i = 0; i != map->objects_number; ++i) {
		...
		if (obj->type == BOSS) {
			...
			if (boss->radius / player->radius < kEatableRatio && !CanUse(map, player, DASH) 
				...
				use.value = 100000;
				use.msg = "Find BOSS!!!";
				use.des = Displacement(player->pos, boss->pos);
	...
	if (CanUse(map, player, SHORT_ATTACK)
		...
		actions.push_back(use);
	} else if (boss && NoOtherPlayerInSight(map)) {
		...
		if (CanUse(map, player, SHORT_ATTACK)
			...
			boss_dis < kLongAttackRange[player->skill_level[LONG_ATTACK]]) {
			use.value = 100000;
			use.skill = LONG_ATTACK;
			use.target = boss->id;
			use.msg = "Attack BOSS";
		...
		&& (nearest_enemy->long_attack_casting || nearest_enemy_dis < kShortAttackRange[5])) {
		use.value = 100000;
		use.skill = SHIELD;
